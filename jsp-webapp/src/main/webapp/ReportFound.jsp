...
<body>
  <div class="container">
    <div class="card">
      ...
      <p class="subtitle">Let the owner know where the item was found so it can be returned safely.</p>

      <c:if test="${not empty successMessage}">
        <div class="success-msg">${successMessage}</div>
      </c:if>

      <form action="${pageContext.request.contextPath}/reportFound" method="post" enctype="multipart/form-data">
        <!-- Item Name -->
        ...
        <div class="grid-2">
          ...
          <div class="form-group">
            ...
            <input
              ...
              name="dateFound"
              max="<%= today %>"
              value="${param.dateFound}"
              class="${not empty errors.dateFound ? 'error' : ''}"
            />
            <c:if test="${not empty errors.dateFound}">
              <div class="error-msg">${errors.dateFound}</div>
            </c:if>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <input
            ...
            name="locationFound"
            maxlength="200"
            placeholder="e.g. Cafeteria, Main Gate"
            value="${fn:escapeXml(param.locationFound)}"
            class="${not empty errors.locationFound ? 'error' : ''}"
          />
          <c:if test="${not empty errors.locationFound}">
            <div class="error-msg">${errors.locationFound}</div>
          </c:if>
        </div>

        <!-- Drop-off Location -->
        <div class="form-group">
          <label for="dropoff">Drop-off Location (optional)</label>
          <input
            id="dropoff"
            type="text"
            name="dropoff"
            ...
            value="${fn:escapeXml(param.dropoff)}"
          />
        </div>
